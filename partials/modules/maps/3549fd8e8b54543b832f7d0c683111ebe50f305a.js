...
//      }
     ...
     // no caching...
      self.getMapAssgnList(function(startedList) {
        ...
        for (var x=0; x < startedList.length; x++) {
          self.loadAssignedMap(startedList[x].$value, function(mapId, mapRef){
            self.getMapAssgn(mapId.mapId, function(fbAssgnRef) {
              mapRef.assgn = fbAssgnRef;
              self.startedMaps.push({id:mapId.mapId, data:mapRef});
              startedCtr++;
              if (startedCtr==startedList.length) {
                cb(self.startedMaps);
                return;
              }
            }.bind({mapRef:mapRef, cb:cb}));
          });
        }
      });
    ...
    self.getMapAssgn = function(mapId, cb) {
      ...
      console.log(mapId);
      console.log("Getting assgn url:" + self.fbAssgnUrl + 'active/'+mapId);
