...
(function(){
  ...
  .service('mapsService', [
   ...
   function ($q, $firebaseObject, $firebaseArray, $log, authService, $filter) {
     var self = this;
     self.fbMapsUrl = "https://intense-torch-339.firebaseio.com/maps/";
     self.fbAddressUrl = "https://intense-torch-339.firebaseio.com/addresses/";
     self.fbAssgnUrl = "https://intense-torch-339.firebaseio.com/assignedMaps/";
     self.fbActiveMapsUrl = "https://intense-torch-339.firebaseio.com/activeMaps";
     self.fbFsgUrl = "https://intense-torch-339.firebaseio.com/fsg/";
     self.fbUsersUrl = "https://intense-torch-339.firebaseio.com/users/";
     self.fbStatusUrl = "https://intense-torch-339.firebaseio.com/status";
     self.phoneWitnoUrl = "https://intense-torch-339.firebaseio.com/phone/bucket";
     self.dncUrl = "https://intense-torch-339.firebaseio.com/phone/bucket";
        ...
        for (var i=0; i<mapObj.addresses.length; i++) {
          ...
          for (var m=0; m<self.phoneStatues.length; m++) {
            if (self.phoneStatues[m] == mapObj.assgn.address[addId]) {
              ...
              removed = true;
              moveAdds.push({addId:addId, "phone");
            }
          }
          ...
          if (moveAdds.length > 0) {
            ...
            var pwArrRef = $firebaseArray(new Firebase(self.dncUrl));
            pwArrRef.$loaded().then(function() {
              for (var m=0; m<moveAdds.length; m++) {
                var moveAddId = moveAdds[m];
                pwArrRef.$add(moveAddId).then(function() {
                  $log.info("Address moved.");
                }, function(err) {
                  $log.error("Failed to put to phone bucket:" + moveAddId);
                });
              }
            });
